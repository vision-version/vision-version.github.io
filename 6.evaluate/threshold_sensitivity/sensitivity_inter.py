...
def eval_ourtool(trueresult_path, sortresults_path):
...
def evaluate(gt_result, our_result):
    ...
    ic(prefect_cve)
    ic(2 * (precision * recall) / (precision + recall))
...
if __name__ == "__main__":
    threshold_result = {}
    inter_weights = ["1", "2", "3", "4", "5"]
    for index, content in enumerate(inter_weights):
        print(content)
        cve_num, prefect_cve, tp_lib_all, fp_lib_all, tn_lib_all, fn_lib_all, precision, recall, f1 = eval_ourtool(f"../trueresult.json", f"sortresults_inter_{content} copy.json")
        threshold_result[content] = {
            "precision": precision,
            "recall": recall,
            "f1-score": f1
        }
    with open("inter_threshold.json", "w") as fw:
        json.dump(threshold_result, fw, indent = 4)