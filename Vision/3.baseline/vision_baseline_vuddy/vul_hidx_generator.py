...
def main():
    ...
    print(originalDir)
    vulsDir = os.path.join(originalDir, "vul_java")
    ...
    for srcFileIdx, returnTuple in enumerate(pool.imap(func, srcFileList)):
        ...
        print(len(functionInstanceList), len(functionInstanceList_New))
        ...
        for fi, f in enumerate(functionInstanceList):
            f.removeListDup()
            path = f.parentFile
            path = "." + path[f.parentFile.find("/vul_java/"):]
    ...
    packageInfo = config.version + ' ' + str(projName) + ' ' + str(numFiles) + ' ' + str(numFuncs) + ' ' + str(
        numLines) + '\n'
    hidxDir = os.path.join(originalDir, "signature_java")
    ...
    print("Hash index saved to:", os.path.join(originalDir, "signature_java", "hashmark_{0}_{1}.hidx".format(intendedAbsLvl, projName)))
    time1 = time.time()
