...
if __name__ == "__main__":
    ...
    for index, content in enumerate(inter_weights):
        print(content)
        cve_num, prefect_cve, tp_lib_all, fp_lib_all, tn_lib_all, fn_lib_all, precision, recall, f1 = eval_ourtool(f"../trueresult.json", f"sortresults_intra_{content} cp.json")
        threshold_result[content] = {
            "precision": precision,
            "recall": recall,
            "f1-score": f1
        }
    with open("intra_threshold.json", "w") as fw:
