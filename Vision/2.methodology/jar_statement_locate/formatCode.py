...
def del_comment(method_contents):
    c_regex = re.compile(
        r'(?P<comment>//.*?$)|(?P<multilinecomment>/\*.*?\*/)|(?P<noncomment>\'(\\.|[^\\\'])*\'|"(\\.|[^\\"])*"|.[^/\'"]*)',
        re.DOTALL | re.MULTILINE,
    )
    method_contents = "".join(
        [
            c.group("noncomment")
            for c in c_regex.finditer(method_contents)
...
def del_lineBreak(method_contents):
    fake_file = StringIO(method_contents)
...
def methodNorm(fileName):
    with open(fileName, "r", errors="ignore") as f:
            file_contents = f.read()
    # ic(file_contents)
    file_contents = del_comment(file_contents)
    file_contents = del_lineBreak(file_contents)
    outputf = open(fileName, "w")
    outputf.write(file_contents)
    outputf.close()
    # addBracket(fileName)


if __name__ == "__main__":
    fileName = "2.methodology/jar_statement_locate/githubshell.java"
    methodNorm(fileName)